...
namespace J2N.Collections.Generic
{
    ...
    /// </summary>
    /// <remarks>
    /// This implementation uses <see cref="SCG.HashSet{T}"/> as a backing set. The only feature that
    /// has been added is a call to <see cref="SCG.HashSet{T}.TrimExcess()"/> when needed to ensure insertion order.
    ...
    [Serializable]
#endif
    public class LinkedHashSet<T> : ICollection<T>, ISet<T>, IReadOnlyCollection<T>, IStructuralEquatable, IFormattable
#if FEATURE_SERIALIZABLE
...
#endif
    {
        ...
        {
            hashSet = new SCG.HashSet<T>(comparer);
        }
        ...
        public void SymmetricExceptWith(IEnumerable<T> other)
        {
            ...
            ExceptWith(other); // NOTE: Don't call the ops on hashSet, we need to track whether trim is needed.
            UnionWith(temp);
        }
