...
#pragma warning restore 168
        ...
        public void TestMinShouldMatch()
        {
...

            {
                ...
                assertEquals(@"0", r.Document(search.ScoreDocs[0].Doc).Get(@"id"));
                assertEquals(new JCG.HashSet<string>(Arrays.AsList(@"2", @"3")), new JCG.HashSet<string>(Arrays.AsList(r.Document(search.ScoreDocs[1].Doc).Get(@"id"), r.Document(search.ScoreDocs[2].Doc).Get(@"id"))));
            }
...

            {
                ...
                TopDocs search = s.Search(query, 10);
                assertEquals(search.TotalHits, 2);
                assertEquals(new JCG.HashSet<string>(Arrays.AsList(@"0", @"2")), new JCG.HashSet<string>(Arrays.AsList(r.Document(search.ScoreDocs[0].Doc).Get(@"id"), r.Document(search.ScoreDocs[1].Doc).Get(@"id"))));
            }
