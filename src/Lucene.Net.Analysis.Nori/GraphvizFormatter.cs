...
namespace Lucene.Net.Analysis.Ko
{
    ...
    public class GraphvizFormatter
    {
        ...
        internal void OnBacktrace(KoreanTokenizer tok, WrappedPositionArray positions, int lastBackTracePos, Position endPosData, int fromIDX, char[] fragment, bool isEnd)
        {
        ...
        private void SetBestPathMap(WrappedPositionArray positions, int startPos, Position endPosData, int fromIDX)
        {
            ...
            while (pos > startPos)
            {
                ...
                if (Debugging.AssertsEnabled) Debugging.Assert(!bestPathMap.ContainsKey(fromNodeID));
                if (Debugging.AssertsEnabled) Debugging.Assert(!bestPathMap.Values.Contains(toNodeID));
        ...
        private string FormatNodes(KoreanTokenizer tok, WrappedPositionArray positions, int startPos, Position endPosData, char[] fragment)
        {
            ...
            for (int pos = endPosData.pos; pos > startPos; pos--)
            {
                ...
                for (int idx = 0; idx < posData.count; idx++)
                {
                    ...
                    string attrs;
                    if (toNodeID.Equals(bestPathMap[fromNodeID]))
                    {
        ...
        {
            return pos.ToString(NumberFormatInfo.InvariantInfo) + "." + idx.ToString(NumberFormatInfo.InvariantInfo);
        }
    }
}
