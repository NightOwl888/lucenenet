...
namespace Lucene.Net.Analysis.Ko
{
    ...
    public sealed class KoreanTokenizer : Tokenizer
    {
        ...
        private readonly List<Token> pending = new List<Token>();
            ...
            while (true)
            {
                ...
                if (userFST != null)
                {
                    ...
                    for (int posAhead = pos; ; posAhead++)
                    {
                        ...
                        output += (int)arc.Output.GetValueOrDefault();
                        if (arc.IsFinal)
                        {
                            maxPosAhead = posAhead;
                            outputMaxPosAhead = output;
                            arcFinalOutMaxPosAhead = (int)arc.NextFinalOutput.GetValueOrDefault();
                            anyMatches = true;
                        }
                    }
                ...
                if (!anyMatches)
                {
                    ...
                    for (int posAhead = pos; ; posAhead++)
                    {
                        ...
                        if (arc.IsFinal)
                        {
                            dictionary.LookupWordIds(output + (int)arc.NextFinalOutput.GetValueOrDefault(), wordIdRef);
                            if (VERBOSE)
                            {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            for (int i = morphemes.Length - 1; i >= 0; i--)
                            {
                                ...
                                {
                                    Debug.Assert(endOffset - morpheme.surfaceForm.Length >= 0);
