...
namespace Lucene.Net.Analysis.Ko.Dict
{
    ...
    public abstract class BinaryDictionary : IDictionary
    {
        ...
        {
            string currentPath = SystemProperties.GetProperty("nori.data.dir", AppDomain.CurrentDomain.BaseDirectory);
        ...
        public static Stream GetTypeResource(Type clazz, string suffix)
        {
            ...
            {
                Stream @is = clazz.Assembly.FindAndGetManifestResourceStream(clazz, fileName);
                if (@is == null)
        ...
        {
            return buffer.GetInt16(wordId) >> 2;
        }

        public virtual int GetRightId(int wordId)
        {
            return buffer.GetInt16(wordId + 2) >> 2; // Skip left id
        }
