...
namespace Lucene.Net.Analysis.Ko.Dict
{
    ...
    public sealed class UserDictionary : IDictionary
    {
        ...
        private UserDictionary(List<string> entries)
        {
            ...
            List<int[]> segmentations = new List<int[]>(entries.Count);
            List<short> rightIds = new List<short>(entries.Count);
            long ord = 0;
        ...
        public IList<int> Lookup(char[] chars, int off, int len)
        {
            ...
            for (int startOffset = off; startOffset < end; startOffset++)
            {
                ...
                for (int i = 0; i < remaining; i++)
                {
                    ...
                    if (arc.IsFinal)
                    {
                        int finalOutput = output + (int)arc.NextFinalOutput.GetValueOrDefault();
