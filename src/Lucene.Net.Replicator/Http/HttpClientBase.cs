...
namespace Lucene.Net.Replicator.Http
{
    ...
    public abstract class HttpClientBase : IDisposable
    {
        ...
        //private readonly HttpWebRequest httpc;
        ...
        //    : this(url/*, (HttpWebRequest)WebRequest.Create(url)*/ /*new HttpClient(messageHandler ?? new HttpClientHandler()) { Timeout = TimeSpan.FromMilliseconds(DEFAULT_CONNECTION_TIMEOUT) }*/)
        //{
        //}
        ...
        //Note: LUCENENET Specific
        protected HttpClientBase(string url /*, HttpWebRequest client*/)
        {
            Url = url;
            //httpc = client;
        ...
        protected virtual void VerifyStatus(HttpWebResponse response)
        {
            //if (!response.IsSuccessStatusCode)
            if ((response.StatusCode < HttpStatusCode.OK) || (response.StatusCode > ((HttpStatusCode)0x12b)))
            {
        ...
        /// <exception cref="IOException">IO Error happened at the server, check inner exception for details.</exception>
        /// <exception cref="HttpRequestException">Unknown error received from the server.</exception>
        protected virtual void ThrowKnownError(HttpWebResponse response)
        {
            Stream input;
            try
            {
            ...
            {
                //not likely
                throw new HttpRequestException(string.Format("Failed to read exception object: {0} {1}", response.StatusCode, response.StatusDescription), e);
            }
            finally
            {
                input.Dispose();
            ...
            throw new HttpRequestException(string.Format("unknown exception: {0} {1}", response.StatusCode, response.StatusDescription), exception);
        }
        ...
        /// </summary>
        protected virtual HttpWebResponse ExecutePost(string request, object entity, params string[] parameters)
        {
            EnsureOpen();
            ...
            var req = (HttpWebRequest)WebRequest.Create(QueryString(request, parameters));

            var content = Encoding.UTF8.GetBytes(JToken.FromObject(entity, JsonSerializer.Create(ReplicationService.JSON_SERIALIZER_SETTINGS))
        ...
        /// The <paramref name="parameters"/> argument is treated as: name1,value1,name2,value2,...
        /// </summary>
        protected virtual HttpWebResponse ExecuteGet(string request, params string[] parameters)
        {
            EnsureOpen();
        ...
        public virtual Stream ResponseInputStream(HttpWebResponse response)
        {
        ...
        /// </summary>
        /// <exception cref="IOException"></exception>
        public virtual Stream ResponseInputStream(HttpWebResponse response, bool consume)
        {
        ...
        /// </summary>
        protected virtual T DoAction<T>(HttpWebResponse response, Func<T> call)
        {
        ...
        /// </summary>
        protected virtual T DoAction<T>(HttpWebResponse response, bool consume, Func<T> call)
        {
    ...
    //        //      and have these Sync methods with their caveats.
    //        HttpResponseMessage response = httpc.SendAsync(req).ConfigureAwait(false).GetAwaiter().GetResult();
    //        //HttpResponseMessage response = httpc.SendAsync(req).Result;
    //        VerifyStatus(response);
    //        return response;
    //    }
