...
namespace Lucene.Net.Replicator.Http
{
    ...
    public class ReplicationHttpListener
    {
        ...
        public ReplicationHttpListener(IReplicationService replicationService, IPAddress ipAddress, int port)
            //: this(replicationService, new HttpListener() { Prefixes = { } })
        {
            ...
            this.httpListener.Prefixes.Add(string.Format("http://{0}:{1}/", ipAddress.ToString(), port));
        }
        ...
        public ReplicationHttpListener(IReplicationService replicationService, HttpListener httpListener)
        {
            if (replicationService == null)
                throw new ArgumentNullException("replicationService");
            if (httpListener == null)
                throw new ArgumentNullException("httpListener");
        ...
        /// </summary>
        public virtual HttpListener HttpListener { get { return this.httpListener; } }
        ...
        /// </remarks>
        /// <param name="httpListenerContext">A <see cref="HttpListenerContext"/> instance.</param>
        protected virtual void ProcessClient(object httpListenerContext)
        {
            var context = ((HttpListenerContext)httpListenerContext);

            context.Response.StatusCode = 200;
            context.Response.SendChunked = true;

            if (context.Request.HttpMethod.ToUpper() == "GET" || context.Request.HttpMethod.ToUpper() == "POST")
            {
                try
                {
                    ...
                    replicationService.Perform(
                        new ReplicationHttpRequest(context.Request), 
                        new ReplicationHttpResponse(context.Response, context.Response.OutputStream));
            ...
            //using (var stream = context. tcpClient.GetStream())
            //{
