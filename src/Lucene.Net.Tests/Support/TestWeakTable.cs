...
namespace Lucene.Net.Support
{
    ...
    [TestFixture]
    public class TestWeakTable
    {
        [Test, LuceneNetSpecific]
        public void A_TestBasicOps()
        {
            WeakTable<object, object> weakDictionary = TestWeakTableBehavior.CreateDictionary();// new SupportClass.TjWeakHashTable();
            ...
            foreach (SmallObject key in keys)
            {
                Assert.AreEqual(((SmallObject)realHashTable[key]).i,
                                ((SmallObject)weakDictionary.GetOrCreateValue(key)).i);

                Assert.IsTrue(realHashTable[key].Equals(weakDictionary.GetOrCreateValue(key)));
            }


            int values1Count = weakDictionary.Count();
            ICollection values2 = (ICollection)realHashTable.Values;
            Assert.AreEqual(values1Count, values2.Count);
            ...
            foreach (SmallObject o in keys2)
            {
                Assert.AreEqual(((SmallObject)realHashTable[o]).i,
                                ((SmallObject)weakDictionary.GetOrCreateValue(o)).i);
                Assert.IsTrue(realHashTable[o].Equals(weakDictionary.GetOrCreateValue(o)));
            }
        }
        ...
        {
            var wht = TestWeakTableBehavior.CreateDictionary();
            int OOMECount = 0;
...
#pragma warning restore 168
        ...
        public void C_TestMemLeakage()
        {
            ...
            var wht = TestWeakTableBehavior.CreateDictionary(); //new SupportClass.TjWeakHashTable();
